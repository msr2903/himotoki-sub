/**
 * Incremental SHA-256 (FIPS 180-4). WebCrypto's digest() needs the whole input in memory; the
 * dictionary import streams ~185 MB through the worker, so we hash chunk by chunk instead.
 */
...
export class Sha256 {
  ...
  /** Lower-case hex digest. The instance must not be reused afterwards. */
  hex(): string {
