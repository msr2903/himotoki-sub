...
### New words only (beta)
`$newWordsLevel` (off / n3 / n2 / n1, `src/shared/newWordsOnly.ts`) replaces the subtitle line with `NewWordsGlossary` (in `Subs.tsx`): one row per Japanese word the learner likely doesn't know (JLPT above the level, or unlisted and rarer than `RARE_RANK`; learning words included, known/ignored excluded) with a short gloss. At most `GLOSSARY_MAX` rows (hardest kept, "+N more"), held `GLOSSARY_HOLD_MS` after the line ends. Needs `$dictReady`. The Show line chip sits beside the glossary and is visible only while the pointer moves over the video (`usePointerActivity`, `SHOW_LINE_IDLE_MS`); it and a click on the glossary background reveal the current cue until the next one (a line with no new words shows only the chip). H and the `showLine` mouse action (`$listeningPeek`) show it until toggled.
